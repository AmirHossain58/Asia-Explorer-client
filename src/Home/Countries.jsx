...
const Countries = () => {
  ...
  useEffect(() => {
    fetch("https://b9-a10-asia-explorer-server.vercel.app/countriesData")
      .then((res) => res.json())
  ...
  return (
    <div>
      <div className="text-center space-y-3 mb-8 my-14">
        ...
        <p>
          {" "}
          Explore diverse countries from around the globe, each with its own
          unique charm and beauty
        </p>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 justify-center gap-5 ">
        {countriesData.map((spot,i) => (
          <CountriesSpot key={i} spot={spot}></CountriesSpot>
        ))}
      </div>
    </div>
  );
